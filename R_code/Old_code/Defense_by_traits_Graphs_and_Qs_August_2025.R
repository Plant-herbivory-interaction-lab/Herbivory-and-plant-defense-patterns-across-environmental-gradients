...
make_plots <- function(data, x_var, y_vars, ncol = 2) {
  plots <- lapply(seq_along(y_vars), function(i) {
    ...
    p <- ggplot(data, aes(x = .data[[x_var]], y = .data[[y]])) +
      geom_point() +
      C_theme +
      labs(y = y_lab, x = NULL)  # no per-panel x-axis title
  ...
  label_plot <- ggplot() + 
    ...
    labs(title = x_var) +        # your centered label
    theme(
      plot.title = element_text(hjust = 0.5)  # center it
    )
  
  wrap_elements(wrap_elements(wrap_plots(plots, ncol = ncol))/ label_plot +
    plot_layout(heights = c(0.9, 0)))
}
