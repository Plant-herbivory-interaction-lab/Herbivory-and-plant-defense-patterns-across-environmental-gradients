require(vegan)
PCbiplot <- function(data1=data,rot_x=1,rot_y=1,font_size=14) {
  PC<-data1  %>% 
    drop_na() %>% 
  ...
  plot <- plot + geom_text(data=datapc, aes(x=PC1*1.9, y=PC2*1.9, label=varnames), color="black",angle = angle, hjust = hjust)
  plot <- plot + 
    geom_segment(data=datapc, aes(x=0, y=0, xend=PC1*2, yend=PC2*2), 
