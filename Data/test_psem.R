test_data<-Data_prep() %>% 
  select(Pop,Year,NPP_y,NPP_g,grep("sc",colnames(.),value=T),herb_p)
...
test_mod<-psem(
  glmmTMB(herb_p_t_sc ~ NPP_y + Latitude_sc + Latitude_sc_sq + Trichomes_t_sc + Conc_t_sc + SLA_t_sc + (1|Year/Pop),data = test_data),
  glmmTMB(Conc_t_sc ~ NPP_y + Latitude_sc + Latitude_sc_sq + (1|Year/Pop),data = test_data),
  glmmTMB(SLA_t_sc ~ NPP_y + Latitude_sc + Latitude_sc_sq + (1|Year/Pop),data = test_data),
  glmmTMB(Trichomes_t_sc ~ NPP_y + Latitude_sc + Latitude_sc_sq + (1|Pop:Year), data = test_data),
  glmmTMB(NPP_y ~ Latitude_sc + (1|dummy), data = test_data),
  SLA_t_sc %~~% Trichomes_t_sc,
  SLA_t_sc %~~% Conc_t_sc,
  data = test_data
